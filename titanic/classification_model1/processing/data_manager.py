...
from classification_model1.config.core import config, DATASET_DIR, TRAINED_MODEL_DIR
...
def pre_pipeline_preparation(*, dataframe: pd.DataFrame):
...
def load_dataset(*, file_name: str) -> pd.DataFrame:
    df = pd.read_csv(Path(f"{DATASET_DIR}/{file_name}"))
    transformed_df = pre_pipeline_preparation(dataframe=df)
